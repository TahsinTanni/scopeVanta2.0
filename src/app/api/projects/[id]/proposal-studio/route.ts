...
export const POST = withErrors(async (req: Request, { params }: { params: Promise<{ id: string }> }) => {
  ...
  await requireEntitlement(ctx.workspaceId, "using Proposal Studio");

  try {
    const r = await aiGenerate({
      track: { workspaceId: ctx.workspaceId, userId: ctx.userId, feature: "proposal-studio" },
      system:
